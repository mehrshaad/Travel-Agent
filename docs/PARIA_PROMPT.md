...
10. Work on a branch named `lane-c/<task>` — one branch per task, never one per lane.
...
2. Show me `git status` and `git diff --stat` so I can confirm nothing outside
   lib/agents/ and lib/llm/ changed.
...
4. Commit with our format: `<type>(c): <what changed>`, e.g.
   `feat(c): add Overpass POI provider with tile cache`.
